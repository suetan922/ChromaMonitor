...
from .ui.main_window import non_dock_theme as mw_non_dock_theme
...
def _sync_toolbar_button_geometry(main_window) -> None:
    """split button の高さを Start / Stop ボタンへ揃える。"""
...
def _build_control_widgets_with_defaults(main_window) -> None:
    """現行 control widget builder で非ドック設定 UI を構築する。"""
...
class MainWindow(QMainWindow):
    ...
    def _build_menu_bar(self) -> None:
        ...
        self.act_open_canvas_preview = tools_menu.addAction("キャンバスプレビュー")
        self.act_open_canvas_preview.triggered.connect(self.show_canvas_preview_window)
    ...
    def _build_toolbar(self) -> None:
        ...
        self.btn_load_image_bar = SplitMenuToolButton()
        self.btn_load_image_bar.setObjectName("fileLoadSplitButton")
        self.btn_load_image_bar.setText("画像読み込み")
        self.btn_load_image_bar.setToolButtonStyle(Qt.ToolButtonTextOnly)
        self.btn_load_image_bar.setPopupMode(SplitMenuToolButton.MenuButtonPopup)
        self.btn_load_image_bar.clicked.connect(self.on_load_image)
        load_menu = QMenu(self.btn_load_image_bar)
        self.act_load_image_from_clipboard = load_menu.addAction("クリップボードから読み込み")
        self.act_load_image_from_clipboard.triggered.connect(self.on_load_image_from_clipboard)
        self._ensure_menu_popup_width(load_menu)
        self.btn_load_image_bar.setMenu(load_menu)
        ...
        QTimer.singleShot(0, lambda mw=self: _sync_toolbar_button_geometry(mw))
    ...
    def _setup_preview_and_docks(self) -> None:
        ...
        self._setup_image_input_drop_targets()
    ...
    _connect_control_signals = mw_controls_signals.connect_control_signals
    ...
    _fit_top_level_widget_to_desktop = mw_windowing.fit_top_level_widget_to_desktop
    ...
    show_canvas_preview_window = mw_tools.show_canvas_preview_window
    _close_canvas_preview_window = mw_tools.close_canvas_preview_window
    ...
    can_accept_image_drop_target = mw_runtime.can_accept_image_drop_target
    is_supported_image_path = mw_runtime.is_supported_image_path
    _setup_image_input_drop_targets = mw_runtime.setup_image_input_drop_targets
    on_image_files_dropped = mw_runtime.on_image_files_dropped
    on_load_image = mw_runtime.on_load_image
    on_load_image_from_clipboard = mw_runtime.on_load_image_from_clipboard
    ...
    _apply_capture_source = mw_runtime.apply_capture_source
    ...
    def _apply_ui_style(self, theme_name: str | None = None):
        """010 の dock スタイルを適用し、その後で非ドック要素だけへ追加テーマ反映する。"""
        if theme_name is not None:
            self._ui_theme_name = str(theme_name)
        mw_windowing.apply_ui_style(self)
        mw_non_dock_theme.apply_additional_theme(self, getattr(self, "_ui_theme_name", None))
    ...
    apply_theme_settings = mw_settings.apply_theme_settings
